...
def post_edit(path):
    ...
    mask_field = cv2.add(
        mask_field, cv2.subtract(cv2.bitwise_not(mask_field), mask_tree)
    )
    ...
    contours_tree, _ = cv2.findContours(
        contour_mask_t, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    contours_field, _ = cv2.findContours(
        contour_mask_f, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    contours_road, _ = cv2.findContours(
        contour_mask_r, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    contours_water, _ = cv2.findContours(
        contour_mask_w, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
